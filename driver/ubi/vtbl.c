...
#ifndef CONFIG_MTD_UBI_READONLY
/**
...
#ifdef CONFIG_MTD_UBI_READONLY
                        /* We can't create the 2nd vtable, so just copy the first one to the second */
                        memcpy(leb[1], leb[0], ubi->vtbl_size);
                        err = 0;
#else
...
#ifdef CONFIG_MTD_UBI_READONLY
                /* We can't create the 2nd vtable, so just copy the good one to the bad one */
                memcpy(leb[0], leb[1], ubi->vtbl_size);
                err = 0;
#else
...
int ubi_read_volume_table(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	if (!av) {
		...
		if (ai->is_empty) {
#ifdef CONFIG_MTD_UBI_READONLY
                        /* We can't create an empty lvol, so declare this an error and give up */
                        ubi_err(ubi, "cannot create an empty lvol when CONFIG_MTD_UBI_READONLY defined");
                        return -EINVAL;
#else
